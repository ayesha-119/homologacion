...
class ProductMatchingPipeline:
    ...
    def process(self, distributor_df):
        ...
        matched_results = df["Clean Name"].apply(lambda x: FuzzyMatcher.match_name(x, self.master_df))
        df["Matched Name"] = matched_results.apply(lambda x: x[0])
        df["Match Score"] = matched_results.apply(lambda x: x[1])
        df["CodigoProducto"] = matched_results.apply(lambda x: x[2])
